...
def get_func_name(func_list: list,
                  ...
                  prompt: str, func_lookup: dict, allowed_token: list) -> Func:
...
def extract_params(model: ModelInterface, func_lookup: dict,
                   func: Func, param_prompt: list, numeral_tokens: list):
...
def process_parameters(model: ModelInterface, funcs: list[Func],
    ...
    for func in funcs:
        extract_params(model, func_lookup, func, param_prompt, numeral_tokens)


def run_model() -> None:
    """run the program from using the input prompts and function definitions
